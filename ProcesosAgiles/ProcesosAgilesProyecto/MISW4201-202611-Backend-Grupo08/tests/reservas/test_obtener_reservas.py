...
from modelos import Usuario, Propiedad, Reserva, Banco, ReservaSchema, db
...
class TestObtenerReservas:
    ...
    def test_retorna_200_si_admin_accede_via_id_admin(self, client):
        propiedad = Propiedad(nombre_propiedad='Casa admin', ciudad='Medellin',
                              direccion='Calle 1 #2-3', nombre_propietario='Propietario Test',
                              numero_contacto='3001234567', banco=Banco.BANCOLOMBIA,
                              numero_cuenta='111222333', id_usuario=self.usuario_2.id,
                              id_admin=self.usuario_1.id)
        db.session.add(propiedad)
        db.session.commit()
        token_admin = create_access_token(identity=str(self.usuario_1.id))
        self.actuar(client, propiedad.id, token=token_admin)
        assert self.respuesta.status_code == 200

    def test_retorna_reservas_si_admin_accede_via_id_admin(self, client):
        reserva_schema = ReservaSchema()
        propiedad = Propiedad(nombre_propiedad='Casa admin', ciudad='Medellin',
                              direccion='Calle 1 #2-3', nombre_propietario='Propietario Test',
                              numero_contacto='3001234567', banco=Banco.BANCOLOMBIA,
        ...
        db.session.add(propiedad)
        db.session.commit()
        reserva = Reserva(nombre='Cliente Test', fecha_ingreso=datetime.strptime('2024-03-01', '%Y-%m-%d'),
                          fecha_salida=datetime.strptime('2024-03-05', '%Y-%m-%d'), plataforma_reserva='Airbnb',
                          total_reserva=300000, comision='40000', id_propiedad=propiedad.id)
        db.session.add(reserva)
        db.session.commit()
        token_admin = create_access_token(identity=str(self.usuario_1.id))
        self.actuar(client, propiedad.id, token=token_admin)
        assert isinstance(self.respuesta_json, list)
        assert [reserva_schema.dump(reserva)] == self.respuesta_json

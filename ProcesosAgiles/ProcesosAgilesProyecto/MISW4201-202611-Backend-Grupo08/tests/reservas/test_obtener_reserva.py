...
from modelos import Usuario, Propiedad, Reserva, ReservaSchema, Banco, db
...
class TestObtenerReserva:
    ...
    def test_retorna_200_si_admin_accede_via_id_admin(self, client):
        propiedad = Propiedad(nombre_propiedad='Casa admin', ciudad='Cali',
                              direccion='Av 1 #5-10', nombre_propietario='Propietario Test',
                              numero_contacto='3009876543', banco=Banco.BANCOLOMBIA,
                              numero_cuenta='999888777', id_usuario=self.usuario_2.id,
                              id_admin=self.usuario_1.id)
        db.session.add(propiedad)
        db.session.commit()
        reserva = Reserva(nombre='Cliente Admin Test', fecha_ingreso=datetime.strptime('2024-04-01', '%Y-%m-%d'),
                          fecha_salida=datetime.strptime('2024-04-05', '%Y-%m-%d'), plataforma_reserva='Booking',
                          total_reserva=450000, comision='60000', id_propiedad=propiedad.id)
        db.session.add(reserva)
        db.session.commit()
        token_admin = create_access_token(identity=str(self.usuario_1.id))
        self.actuar(client, reserva.id, token=token_admin)
        assert self.respuesta.status_code == 200

...
class TestObtenerMovimiento:
    ...
    def test_retorna_200_si_admin_accede_via_id_admin(self, client):
        ...
        db.session.commit()
        movimiento = Movimiento(fecha=datetime.strptime('2023-02-01', '%Y-%m-%d'), valor=5000.0,
                                concepto='Pago adicional', tipo_movimiento=TipoMovimiento.INGRESO,
                                id_propiedad=propiedad.id)
        db.session.add(movimiento)
        db.session.commit()
        token_admin = create_access_token(identity=str(self.usuario_1.id))
        self.actuar(client, movimiento.id, token_admin)
        assert self.respuesta.status_code == 200

...
class TestObtenerMovimientos:
    ...
    def test_retorna_200_si_admin_accede_via_id_admin(self, client):
        ...
        self.actuar(client, propiedad.id, token_admin)
        assert self.respuesta.status_code == 200

    def test_retorna_movimientos_de_propiedad_si_admin_accede_via_id_admin(self, client):
        movimiento_schema = MovimientoSchema()
        propiedad = Propiedad(nombre_propiedad='Casa admin', ciudad='Medellin',
                              direccion='Calle 1 #2-3', nombre_propietario='Propietario Test',
                              numero_contacto='3001234567', banco=Banco.BANCOLOMBIA,
        ...
        db.session.add(propiedad)
        db.session.commit()
        movimiento = Movimiento(fecha=datetime.strptime('2023-02-01', '%Y-%m-%d'), valor=9000.0,
                                concepto='Gasto extra', tipo_movimiento=TipoMovimiento.EGRESO,
        ...
        assert movimiento_schema.dump(movimiento) in self.respuesta_json

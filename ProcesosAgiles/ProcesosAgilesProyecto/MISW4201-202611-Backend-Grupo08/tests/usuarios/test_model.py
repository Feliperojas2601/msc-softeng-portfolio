...
class TestCreateUser(TestCase):
    ...
    def test_solo_un_usuario_es_creado_en_db(self):
        new_user = Usuario(usuario='test_user', rol=Usuario.ROL_PROPIETARIO, contrasena='123456')
    ...
    def test_usuario_esperado_es_creado(self):
        new_user = Usuario(usuario='test_user', rol=Usuario.ROL_ADMIN, contrasena='123456')
        db.session.add(new_user)
        db.session.commit()
    ...
    def test_dispara_un_error_de_integridad(self):
        first_user = Usuario(usuario='test_user', rol=Usuario.ROL_PROPIETARIO, contrasena='123456')
        db.session.add(first_user)
        db.session.commit()
        with self.assertRaises(exc.IntegrityError):
            user_same_username = Usuario(usuario='test_user', rol=Usuario.ROL_ADMIN, contrasena='abcdef')

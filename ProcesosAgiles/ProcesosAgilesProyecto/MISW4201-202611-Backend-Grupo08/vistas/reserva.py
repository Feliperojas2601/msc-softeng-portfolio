...
class VistaReserva(Resource):
    ...
    def put(self, id_reserva):
        ...
        reserva_schema.load(request.json, session=db.session, instance=resultado_buscar_reserva.entidad, partial=True)
        db.session.commit()
        return reserva_schema.dump(resultado_buscar_reserva.entidad)

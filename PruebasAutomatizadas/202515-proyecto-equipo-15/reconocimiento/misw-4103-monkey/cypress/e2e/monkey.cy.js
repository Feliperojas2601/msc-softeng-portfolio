...
describe("monkey", () => {
  ...
  before(() => {
    ...
    if (hash.includes("setup")) {
        cy.addActionContext({ title: "Registro inicial de Ghost", value: hash });
        cy.get('input[id="blog-title"]').should("be.visible").type("Monkey Ghost Site", { delay: 50 });
        cy.get('input[id="name"]').should("be.visible").type("Test Monkey", { delay: 50 });
        cy.get('input[id="email"]').should("be.visible").type(email, { delay: 50 });
        cy.get('input[id="password"]').should("be.visible").type(password, { delay: 50 });
        cy.get('button[data-test-button="setup"]').should("be.enabled").click();
        cy.wait(8000);
        cy.addActionContext({ title: "Registro completado", value: email });
    } else if (hash.includes("signin")) {
        cy.addActionContext({ title: "Inicio de sesión en Ghost", value: hash });
        cy.get('input[name="identification"], input[id="identification"]').should("be.visible")
        .type(email, { delay: 50 });
        cy.get('input[name="password"], input[id="password"]').should("be.visible")
        .type(password, { delay: 50 });
        cy.get('button.login, button[type="submit"]').should("be.enabled").click();
        cy.wait(4000);
        cy.addActionContext({ title: "Inicio de sesión completado", value: email });
    } else {
        cy.addActionContext({ title: "Sesión activa detectada", value: hash });
    }
});

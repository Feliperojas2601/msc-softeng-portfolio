...
When('I introduce the tag name and slug', async function () {
    ...
    await this.driver.pause(3000);
    const tagNameField = await this.driver.$('input[data-test-input="tag-name"]');
    ...
    const tagSlugField = await this.driver.$('input[data-test-input="tag-slug"]');
...
When('I introduce the tag description', async function () {
    ...
    await this.driver.pause(3000);
    const tagDescriptionField = await this.driver.$('textarea[data-test-input="tag-description"]');
